...
namespace App\Tests\Integration\Loyalty\Infrastructure\Projection;
...
use App\Loyalty\Infrastructure\Projection\WalletProjection;
...
#[CoversClass(WalletProjection::class)]
final class WalletProjectionTest extends IntegrationTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->projection = new WalletProjection($this->connection, $this->notifier);
    ...
    public function it_creates_a_wallet_row_with_a_zero_balance(): void
    {
        ...
        $this->projection->projectWalletCreated(new WalletCreated($walletId->value, $this->now()));
    ...
    public function it_tracks_the_balance_across_the_full_transfer_lifecycle(): void
    {
        ...
        $this->projection->projectWalletCreated(new WalletCreated($walletId->value, $this->now()));
        $this->projection->projectPointsAdded(new PointsAdded($walletId->value, 300, null, $this->now()));
        $this->projection->projectPointsWithdrawn(new PointsWithdrawn($walletId->value, 120, $transferId->value, $this->now()));
        $this->projection->projectPointsRefunded(new PointsRefunded($walletId->value, 120, $transferId->value, $this->now()));
    ...
    public function it_records_a_withdrawal_as_a_negative_history_entry(): void
    {
        ...
        $this->projection->projectWalletCreated(new WalletCreated($walletId->value, $this->now()));
        $this->projection->projectPointsWithdrawn(new PointsWithdrawn($walletId->value, 40, $transferId->value, $this->now()));
    ...
    public function it_survives_a_redelivered_creation_event(): void
    {
        ...
        $this->projection->projectWalletCreated($event);
        $this->projection->projectWalletCreated($event);
    ...
    public function it_pushes_the_balance_it_just_wrote(): void
    {
        ...
        $this->projection->projectWalletCreated(new WalletCreated($walletId->value, $this->now()));
        $this->projection->projectPointsAdded(new PointsAdded($walletId->value, 90, null, $this->now()));
    ...
    #[Test]
    public function it_returns_null_for_a_wallet_the_projection_has_not_seen(): void
    {
